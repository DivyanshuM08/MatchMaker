...
We use **MVVM** with a **Repository**. The screen gets data from a ViewModel, which talks only to the repository. The repository is the single source of truth: it reads from Room and, on refresh, fetches from the API and writes to Room. The UI observes a Flow from the repository, so it stays in sync and works offline.
...
The list always comes from Room. When you tap refresh, we fetch from the API and merge into the DB. If the request fails (or our 30% flaky simulation kicks in), we retry a few times with backoff; on success we persist whatever we got. Errors are shown in a Snackbar with a Retry action. We also listen for network changes and show “You’re offline” / “Back online” so the user knows when data might be stale.
